...
import ca.openosp.openo.ws.rest.to.model.AllowedAppointmentTypeTransfer;
...
public class Provider {
    ...
    public static Provider fromProvider(BookingProviderTransfer provider) {
        ...
        if (provider.isFilterEAF()) {
            FilterDefinition fd = new FilterDefinition();
            fd.setFilterClassName("ca.openosp.openo.filters.search.appointment.ExistingAppointmentFilter");
            returnProvider.filters.add(fd);
        }
        if (provider.isFilterMUF()) {
            FilterDefinition fd = new FilterDefinition();
            fd.setFilterClassName("ca.openosp.openo.filters.search.appointment.MultiUnitFilter");
            returnProvider.filters.add(fd);
        }
        ...
        if (provider.isFilterSCTF()) {
            FilterDefinition fd = new FilterDefinition();
            fd.setFilterClassName("ca.openosp.openo.filters.search.appointment.SufficientContiguousTimeFilter");
            returnProvider.filters.add(fd);
        }
