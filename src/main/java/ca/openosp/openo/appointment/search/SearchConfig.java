...
import ca.openosp.openo.ws.rest.to.model.AppointmentTypeTransfer;
import ca.openosp.openo.ws.rest.to.model.BookingProviderTransfer;
import ca.openosp.openo.ws.rest.to.model.BookingScheduleTemplateCodeTransfer;
...
public class SearchConfig {
    ...
    private List<Provider> bookingProviders = null; //needed for ehrweb service delete me
    ...
    public Integer getNumberOfMinutesAdvance() {
        if (filters != null) {
            for (FilterDefinition fd : filters) {
                if ("ca.openosp.openo.filters.search.appointment.FutureApptFilter".equals(fd.getFilterClassName())) {
    ...
    public Character[] getOpenAccessCodes() {
        if (filters != null) {
            for (FilterDefinition fd : filters) {
                if ("ca.openosp.openo.filters.search.appointment.OpenAccessFilter".equals(fd.getFilterClassName())) {
    ...
    public static SearchConfig fromTransfer(SearchConfigTo1 clinicTransfer, SearchConfig oldClinic) {
        ...
        if (clinicTransfer.getNumberOfMinutesAdvance() != null && clinicTransfer.getNumberOfMinutesAdvance() > 0) {
            ...
            fd.setFilterClassName("ca.openosp.openo.filters.search.appointment.FutureApptFilter");
            Map<String, String> params = new HashMap<String, String>();
		...
		/* Check if this is not needed... it seems the above codes receives the hashmap instead of creating it.
		  if(bookingCodeList != null) {
			...
			if(openAccessList.size() > 0){
				FilterDefinition fd = new FilterDefinition();
				fd.setFilterClassName("ca.openosp.openo.filters.search.appointment.OpenAccessFilter");
				returnClinic.filters.add(fd);
			}
		...
		*/
        ...
        fdExistingApp.setFilterClassName("ca.openosp.openo.filters.search.appointment.ExistingAppointmentFilter");
        returnClinic.filters.add(fdExistingApp);

        FilterDefinition fdMultiUnit = new FilterDefinition();
        fdMultiUnit.setFilterClassName("ca.openosp.openo.filters.search.appointment.MultiUnitFilter");
