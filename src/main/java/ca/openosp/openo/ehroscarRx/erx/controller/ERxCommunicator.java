...
import ca.openosp.openo.ehroscarRx.erx.model.PrescriptionFormat;
import ca.openosp.openo.ehroscarRx.erx.model.request.GetPrescriptions5;
import ca.openosp.openo.ehroscarRx.erx.model.request.SetPatientImmediate3;
import ca.openosp.openo.ehroscarRx.erx.model.request.Transaction3;
import ca.openosp.openo.ehroscarRx.erx.model.response.GetPrescriptions5Response;
import ca.openosp.openo.ehroscarRx.erx.model.response.SetPatientImmediate3Response;
                ...
                // Otherwise, parse each document in the response
                    ...
                    while (responseBodyElementIterator.hasNext()) {
                        try {
                            ...
                            logger.error("Failed to parse response because the remote ehrweb service reported that one of the parameters in the request was invalid.\n\nThe request was...\n"
                                    + this.transformSOAPMessageToString(request)
                                    + "\n...and the response was...\n"
                                    + this.transformSOAPMessageToString(response));
                        } catch (ServiceUnavailableException e) {
                            logger.error("Failed to request prescription data because the remote ehrweb service was unavailable: "
                                    + e.getMessage());
                ...
                // Otherwise, parse each document in the response
                    ...
                    while (responseBodyElementIterator.hasNext()) {
                        try {
                            ...
                            logger.error("Failed to send patient data because the remote ehrweb service was unavailable: "
                                    + e.getMessage());
