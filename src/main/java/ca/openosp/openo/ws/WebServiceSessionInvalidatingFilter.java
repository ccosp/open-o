...
/**
 * Our ehrweb services are inherently stateless so we want to prevent excessive session object build up. This is caused because
...
public class WebServiceSessionInvalidatingFilter implements javax.servlet.Filter {
    ...
    public void doFilter(ServletRequest tmpRequest, ServletResponse tmpResponse, FilterChain chain) throws IOException, ServletException {
        ...
        try {
            ...
            //don't apply to REST calls, we want those to be available to the ehrweb interface without losing session
