 ...
 * Ontario, Canada
 */
package openo.form.study.hsfo2;
...
import openo.form.study.hsfo2.pageUtil.ConvertUtil;
...
public class HSFODAO {
    ...
    public List getLabWorkInDateRange(String patientId, String startDate, String endDate, String visitDate) {
        ...
        int count = 0;
        List<Measurement> measurements = measurementDao.findByDemographicIdObservedDate(Integer.valueOf(patientId), DateUtils.toDate(startDate), DateUtils.toDate(endDate));
        for (Measurement m : measurements) {
            if (m.getDateObserved().before(DateUtils.toDate(visitDate)))
                continue;
