    ...
    public List<Object[]> findDocuments(String module, String moduleid, String docType, boolean includePublic,
        ...
        if (includePublic) {
            if (isShowingAllDocuments) {
                ...
                buf.append(" AND d.public1 = 1 AND d.doctype = ?2");
                params.put(2, docType);
            }
        } else {
            if (isShowingAllDocuments) {
                buf.append(" AND c.id.moduleId = ?1 AND d.public1 = 0");
                params.put(1, ConversionUtils.fromIntString(moduleid));
            } else {
                buf.append(" AND c.id.moduleId = ?1 AND d.public1 = 0 AND d.doctype = ?2");
                params.put(1, ConversionUtils.fromIntString(moduleid));
                params.put(2, docType);
            }
        }
