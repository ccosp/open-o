...
public class DxresearchDAOImpl extends AbstractDaoImpl<Dxresearch> implements DxresearchDAO {
    ...
    public List<DxRegistedPTInfo> getPatientRegisted(List<Dxresearch> dList, List<String> doctorList) {
        ...
        while (i.hasNext()) {
            ...
            if (demo != null) {
                ...
                // if patient's doctor is not in the doctor's list then skip, "*" means all
                // doctor
    ...
    public List<DxRegistedPTInfo> patientRegistedStatus(String status, List<dxCodeSearchBean> searchItems,
        ...
        if (listItems != null && listItems.size() > 0) {
            ...
            HQL = "SELECT dxres FROM Dxresearch dxres WHERE dxres.status= '" + status
    ...
    @Override
    public Integer countBillingResearches(String researchCode, String diagCode, String creator, Date sdate,
            Date edate) {
    ...
    public List<Object[]> findResearchAndCodingSystemByDemographicAndCondingSystem(String codingSystem,
        ...
        String sql = "select d.start_date, d.update_date, c.description, c." + codingSystem
                + ", d.dxresearch_no, d.status,d.providerNo from dxresearch d, " + codingSystem + " c " +
                "where d.dxresearch_code=c." + codingSystem + " and d.status<>'D' and d.demographic_no ='"
                + demographicNo + "' and d.coding_system = '" + codingSystem + "'"
    ...
    public List<Dxresearch> findNonDeletedByDemographicNo(Integer demographicNo) {
        Query query = entityManager
                .createQuery("FROM Dxresearch d WHERE d.demographicNo = :dn and (d.status = 'A' or d.status = 'C')");
        query.setParameter("dn", demographicNo);
    ...
    public String getDescription(String codingSystem, String code) {
        ...
        try {
            ...
            @SuppressWarnings("unchecked")
            AbstractCodeSystemDao<AbstractCodeSystemModel<?>> csDao = (AbstractCodeSystemDao<AbstractCodeSystemModel<?>>) SpringUtils
                    .getBean(daoName);
