...
public class TicklerDaoImpl extends AbstractDaoImpl<Tickler> implements TicklerDao {
    ...
    public List<Tickler> findActiveByMessageForPatients(List<Integer> demographicNos, String remString) {
        ...
        Query query = entityManager.createQuery(
                "select t from Tickler t where t.demographicNo in (:demographicNos) and t.status = 'A' and t.message like :msg");
    ...
    public List<Tickler> findActiveByDemographicNoAndMessage(Integer demoNo, String message) {
        ...
        Query query = entityManager.createQuery(
                "select t from Tickler t where t.demographicNo = ?1 and t.message = ?2 and t.status = 'A'");
    ...
    public List<Tickler> findActiveByDemographicNo(Integer demoNo) {
        ...
        Query query = entityManager.createQuery(
                "select t from Tickler t where t.demographicNo = ?1 and t.status = 'A' order by t.serviceDate desc");
        query.setParameter(1, demoNo);
    ...
    public List<Tickler> findByTicklerNoDemo(Integer ticklerNo, Integer demoNo) {
        ...
        Query query = entityManager
                .createQuery("select t from Tickler t where t.id = ?1 AND t.demographicNo = ?2 AND t.status != 'D'");
    ...
    public List<Tickler> findByTicklerNoAssignedTo(Integer ticklerNo, String assignedTo, Integer demoNo) {
        ...
        Query query = entityManager.createQuery(
                "select t from Tickler t where t.id = ?1 AND t.taskAssignedTo = ?2 AND t.demographicNo = ?3 AND t.status != 'D'");
        query.setParameter(1, ticklerNo);
    ...
    /**
     * Finds all ticklers for the specified demographic, assigned to and message
     * fields.
     */
    @SuppressWarnings("unchecked")
    @Override
    public List<Tickler> findByDemographicIdTaskAssignedToAndMessage(Integer demographicNo, String taskAssignedTo,
            String message) {
        Query query = entityManager.createQuery(
                "select t from Tickler t where t.demographicNo = ?1 AND t.taskAssignedTo = ?2 and t.message = ?3");
    ...
    public int getActiveTicklerByDemoCount(Integer demographicNo) {
        Query query = entityManager.createQuery(
                "select count(t) FROM Tickler t where t.status = 'A' and t.serviceDate <= ?1 and t.demographicNo  = ?2 ");
        query.setParameter(1, new Date());
    ...
    private String getTicklerQueryString(String selectQuery, List<Object> paramList, CustomFilter filter) {
        ...
        if (includeMRPClause) {
            query = selectQuery
                    + " FROM Tickler t, Demographic d where d.DemographicNo = t.demographicNo and d.ProviderNo = '"
                    + filter.getMrp() + "' ";
        }
