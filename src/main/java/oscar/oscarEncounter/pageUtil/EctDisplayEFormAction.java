	...
	try
	{
		...
		if(!securityInfoManager.hasPrivilege(LoggedInInfo.getLoggedInInfoFromSession(request), "_eform", "r", null)) {
			...
			for (EFormData eFormData : eFormDatas)
			{
	            ...
	            hash = Math.abs(winName.hashCode());
	            url = "popupPage( 800, 960, '" + hash + "', '" + request.getContextPath() + "/eform/efmshowform_data.jsp?fdid="+eFormData.getId()+"&demographic_no="+bean.demographicNo+"&appointment="+bean.appointmentNo+"&parentAjaxId="+cmd+"');";
